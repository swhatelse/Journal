...
*** TODO 2015-06-18 thursday [0/1]
    - [ ] SegFault surf_resource_get_properties async_ring_implicit:
    ...
    --> #5  0x00007ffff752ef9f in smpi_main (realmain=0x40bcfe <_starpu_mpi_simgrid_init>, argc=3, argv=0x7fffffffda18)
      ...
      - What happens before the simcall:
        #+BEGIN_EXAMPLE
    ...
    --> #4  0x00007ffff7370115 in MSG_host_get_data (host=0x7ffff6b11b78 <main_arena+88>) at /home/steven/Programmation/TER/starpu-simgrid/src/simgrid/src/msg/msg_host.c:99
        ...
        $3 = 1
        #+END_EXAMPLE
        The value return by MSG_host_self() is odd.
        #+BEGIN_EXAMPLE
        #5  0x0000000000420a6e in starpu_pthread_getspecific (key=1) at ../../src/common/thread.c:205
        (gdb) p MSG_host_self()
        $7 = (struct s_xbt_dictelm *) 0x7ffff6b11b78 <main_arena+88>
        #+END_EXAMPLE
      - What MSG_host_self() return after the creation of the process
        of pid 1?
        #+BEGIN_EXAMPLE
        Breakpoint 1, SIMIX_process_create (process=0x7ffff5dbdca0, name=0x4aa622 "main", code=0x404f1a <smpi_simulated_main_>, data=0x7ffff00009b0,
        hostname=0x92d670 "StarPU-MPI0-MAIN", kill_time=-1, argc=1, argv=0x8ec300, properties=0x0, auto_restart=0, parent_process=0x8f4c70)
        at /home/steven/Programmation/TER/starpu-simgrid/src/simgrid/src/simix/smx_process.c:238
        (gdb) p parent_process->pid
        $2 = 1
        (gdb) p MSG_host_self()
        $3 = (struct s_xbt_dictelm *) 0x0
        #+END_EXAMPLE
        In the program callback, MSG_host_self() return also null.
      - At the creation of 1:
        #+BEGIN_EXAMPLE
      ...
      - Having replaced MSG_host_self by SIMIX_host_self -> still
        having a segfault:
        It is because SMPI process' don't have 
        #+BEGIN_EXAMPLE
        Program received signal SIGTRAP, Trace/breakpoint trap.                                                                                                                            
...
**** TODO Compilation problem with libtcmalloc [0/1] : 
     If the libtcmalloc, it will try to use this lib for linking but
     it fails for static as there is no .a.
     Just removed it.
     - [ ] Inform Samuel about this problem.
* Experiments
** TODO [0/3]
   - [ ] Prepare all I need for test and check all is ok
   - [ ] Test on Grid5K
   - [ ] Test with SimGrid
** Real
** Simulation 

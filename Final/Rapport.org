...
** StarPU-SMPI:Ce qui coince
   ...
   Dans SimGrid seul le segment
   données est privatisé, comme les variables globales des librairies
   dynamiques ne se trouvent pas dans ce dernier, elles restent donc
...
* Contribution
  ...
  qui les a créés. SimGrid a donc été modifié en conséquences.
  ...
  La solution qui nous avons employée est d'utiliser donc une version

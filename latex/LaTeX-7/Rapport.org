...
* Méthodologie
  Comme nous travaillons avec Simgrid et StarPU à la fois, afin de
  pouvoir gérer simplement les mises à jours de ces derniers, nous
  utilisons un dépôt complexe comprenant les deux et gérer avec
  l'outils submodule de git. Ce dernier nous
  ...
  laboratoire est tenu en org-mode et hébergé sur github. Cela permet
  ...
  modifications au niveau de Simgrid. Il a été dans un premier temps été
  nécessaire de consulter la documentation afin de comprendre le
  fonctionnement et l'architecture de Simgrid. Ensuite il a fallut
  explorer le code afin de déterminer où apporter les modifications

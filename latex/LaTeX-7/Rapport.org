...
*** SMPI 
    ...
    Le fonctionnement est le suivant :
      ...
      Afin que ces dernier ce comporte comme de processus UNIX, il
      est nécessaire que chaque processus n'est pas accès aux
      variables d'un autre, c'est pourquoi un système de
      privatisation a été mis en place. L'approche est la suivante:
      pour chaque processus, une zone mémoire lui est alloué dans le
      tas grâce a un mmap. Cette zone représente le segment données du
      processus, et a chaque changement de contexte, on fait pointer
      vers cette zone. 
...
** Ce qui coince
   Comme nous l'avions évoqué un peu plus haut, les processus sont
   modélisés par des threads, il faut donc un mécanisme de
   privatisation afin que, comme avec les vrai processus, chacun
   dispose d'un segment data qui soit propre. Sous Simgrid le
   mécanisme est le suivant:

   Comme en MPI on est dans un contexte
   d'espace mémoire distribuée, les processus MPI d'un même noeud
   ...
   Le schéma ci-dessous illustre le fonctionnement de la privatisation
   dans Simgrid.

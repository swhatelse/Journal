...
*** Pas draft
    ...
    mais l'exécution des kernels de calculs et les transferts de
    données sont simulés. 
    Avec cette méthode les expérimentations peuvent être effectuées à
    partir de n'importe quel système, il n'est plus nécessaire d'avoir
    accès à la plateforme, ce qui rend cette approche peu
    coûteuse. Par ailleur il est facile d'extrapoler les résultats car
    ...
    de mesures.  
    Enfin comme la simulation nous permet d'avoir un contrôle sur
    l'ordonnancement, nous pouvons avoir un système déterministe qui
    nous permet d'avoir des expériences qui peuvent être reproduites.
